        ...
        <div class="containerWrap">
          ...
          >
            <div class="titleWrap">
              ...
              <span 
                @click="goOther('/takeLeadReport',item.id,'report')" 
                v-if="item.performOperations == '汇报'">
                <b class="report">汇报</b>
              </span>
              <span
                @click="goOther('/takeLeadReport',item.id,'modify')"
                v-else
              ><b class="modify">修改</b></span>
            </div>
            <div @click="goOther('/takeLeadReportDetail',item.id,'detail')" class="content">
...
export default {
  ...
  methods: {
    ...
    reportBack(id,index) {
      this.$api.takeLeadMission.reportBack(id)
      .then(res=>{
        if(res.code == '200') {
    ...
    getInReport() {
      ...
      this.$api.takeLeadMission.getInReport(data)
      .then(res=>{
    ...
    getReported() {
      ...
      var info = aes.encrypt(JSON.stringify(this.info)),
      data = `"requestData": "${info}"`;
      this.$api.takeLeadMission.getReported(data)
      .then(res=>{
