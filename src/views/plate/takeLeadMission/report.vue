  ...
  <div class="reportWrap">
    ...
    <div class="plateTwo-wrap">
      <div class="plateTwo-cont">
        ...
        <div class="onePlate" v-if="pageType == 'modify'">
        ...
        <div class="onePlate">
          ...
          <div class="text-wrap">
            <p @click="goReportList()">查看汇报情况<van-icon name="arrow" /></p>
          </div>
        </div>
        <div class="onePlate">
...
export default {
  ...
  watch: {
    $route(to,from) {
      if(this.$route.query.takeLeadId && from.path== '/mineMission_hb' && to.path == '/takeLeadReport') {
        // 如果来源于汇报列表 重置数据
        this.reset()
      }
    }
  ...
  methods: {
    reset() {
      this.reportId = this.$route.query.takeLeadId;
    ...
    goReportList() {
      ...
      this.$router.push({path: '/takeLeadReportList',query: {id: id}})
    },
    showPop2(item) {
      this.showTitle = '';
      this.showText = item;
    ...
    getReportDetail() {
      let id = this.reportId;
      this.$api.takeLeadMission.getReportInfo(id)
      .then(res => {
        if(res.code == '200') {
    ...
    getFile(event) { //确认文件后的操作
      ...
      id = this.reportId;
      axios.post(`${base.baseUrl}/app/qtrw/${id}/fileUpload`,formData,config)
      .then(res=>{
        if(res.code == '200') {
          let file = {
    ...
    submit() {
      ...
      var info = aes.encrypt(JSON.stringify(this.info)),
        ...
        id = this.reportId;
      this.$api.takeLeadMission.submitReport(id,data)
      .then(res=>{
        if(res.code == '200') {
          Toast('汇报成功！')
          window.setTimeout(() => {
