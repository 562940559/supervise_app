        ...
        <div class="containerWrap">
          ...
          >
            <div class="titleWrap">
              ...
              <span 
                @click="goOther('/undertakeReport',item.id,'report')" 
                v-if="item.performOperations == '汇报'">
                <b class="report">汇报</b>
              </span>
              <span
                @click="goOther('/undertakeReport',item.id,'modify')"
                v-else
              ><b class="modify">修改</b></span>
            </div>
            <div @click="goOther('/undertakeReportDetail',item.id,'detail')" class="content">
...
export default {
  ...
  methods: {
    ...
    reportBack(id,index) {
      this.$api.undertakeMission.reportBack(id)
      .then(res=>{
        if(res.code == '200') {
    ...
    getInReport() {
      ...
      this.$api.undertakeMission.getInReport(data)
      .then(res=>{
    ...
    getReported() {
      ...
      var info = aes.encrypt(JSON.stringify(this.info)),
      data = `"requestData": "${info}"`;
      this.$api.undertakeMission.getReported(data)
      .then(res=>{
