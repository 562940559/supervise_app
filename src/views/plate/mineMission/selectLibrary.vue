...
export default {
  ...
  methods: {
    ...
    addFile(file) {
      ...
      let evenSelect = JSON.parse(localStorage.getItem('thisSelectLibrary'))
      evenSelect = evenSelect.concat(thisSelect)
      localStorage.setItem('thisSelectLibrary',JSON.stringify(evenSelect))
      Toast('添加成功！')
      this.$router.back('-1')
    },
    onLoad() {
