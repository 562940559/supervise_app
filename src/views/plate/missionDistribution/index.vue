    ...
    <van-tabs @click="onclick" v-model="active">
      <van-tab title="待我分配">
        <div class="containerWrap">
          ...
          <div 
            class="contentWrap"
            v-for="(item,index) in inDistribution"
            :key="index"
            @click="goDetail(item.id,'distribution')"
          >
            <div class="titleWrap">
      ...
      <van-tab title="我已分配">
        <div class="containerWrap">
          ...
          <div 
            class="contentWrap"
            v-for="(item,index) in distributioned"
            :key="index"
...
export default {
  ...
  methods: {
    ...
    goDetail(id,type) {
      this.$router.push({path: '/missionDistributionDetail',query: {id: id,type: type}})
    },
    getInDistribution() {
      this.loading = true;
      this.finished = false;
      ...
      this.$api.missionDistribution.getInDistribution(data)
      .then(res=>{
        if (res.pendingAllocations[0] == undefined) {
          this.loading = false;
          this.finished = true;
          return false;
        }
        this.info.current++;
        this.inDistribution = this.inDistribution.concat(res.pendingAllocations);
        this.loading = false;
      })
    },
    getDistributioned() {
      ...
      var info = aes.encrypt(JSON.stringify(this.info)),
      data = `"requestData": "${info}"`;
      this.$api.missionDistribution.getDistributioned(data)
      .then(res=>{
        if (res.assigneds[0] == undefined) {
          this.loading = false;
          this.finished = true;
          return false;
        }
        this.info.current++;
        this.distributioned = this.distributioned.concat(res.assigneds);
        this.loading = false;
      })
    },
    onclick(name,title) {
      this.info.current = 1;
      this.distributioned = [];this.inDistribution = [];
      this.onLoad()
    },
    onLoad() {
      if(this.active == '0') {
        this.getInDistribution()
      }else if(this.active == '1'){
        this.getDistributioned()
      }
    }
  },
  mounted() { //获取列表信息
