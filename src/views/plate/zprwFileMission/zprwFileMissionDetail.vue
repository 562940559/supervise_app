    ...
    <div class="plateOne-wrap">
      <div class="plateOne-cont">
        <div class="top">
          ...
          <div class="txt-wrap">
            <p @click="showPop2(dataList.taskName)">{{dataList.taskName}}</p>
            <p v-if="fileMissionType == 'todo'">待归档</p>
            <p v-if="fileMissionType == 'done'">已归档</p>
          </div>
        </div>
        <div class="cont">
          <p>开始时间：<span>{{dataList.startTime}}</span></p>
          <p>截止时间：<span>{{dataList.endTime}}</span></p>
          <p>办理人：<span>{{dataList.reporterName}}</span></p>
          <p>提醒模板：<span>{{dataList.reportTemplet}}</span></p>
        </div>
      </div>
    </div>
    <div class="plateTwo-wrap">
      <div class="plateTwo-cont">
        <div class="onePlate">
          ...
          <div class="text-wrap">
            <p @click="showPop('任务内容',dataList.taskContent)">任务内容<van-icon name="arrow" /></p>
          </div>
        </div>
        <div class="onePlate">
          <div class="img-wrap"><img src="../../../../static/img/mineMission/ckbz.png" alt=""></div>
          <div class="text-wrap">
            <p @click="showPop('备注',dataList.remark)">备注<van-icon name="arrow" /></p>
          </div>
        </div>
        <div class="onePlate">
        ...
        <div class="onePlate">
          ...
          <div class="text-wrap">
            <p @click="goOther('/seeReportList')">查看汇报详情<van-icon name="arrow" /></p>
          </div>
        </div>
        <div class="onePlate">
...
export default {
  ...
  methods: {
    getTodoMissionDetail() {
      ...
      this.$api.zprwFileMission.getTodoMissionDetail(id)
      .then(res=>{
        this.dataList = res;
      })
    },
    getDoneMissionDetail() {
      var id = this.fileMissionId;
      this.$api.zprwFileMission.getDoneMissionDetail(id)
      .then(res=>{
        this.dataList = res;
      })
    },
    ...
    goOther(where) {
      if(where == '/seeReportList') {
        localStorage.setItem('zprwReportList',JSON.stringify(this.dataList.reportSheetData))
      }
    ...
    sure() {
      Dialog.confirm({
        ...
        var id = this.fileMissionId;
        this.$api.zprwFileMission.submitFile(id)
        .then(res=>{
          if(res.code == '200') {
            Toast('归档成功！')
            window.setTimeout(() => {
  ...
  mounted() {
    ...
    console.log(this.fileMissionType)
    this.fileMissionId = this.$route.query.id;
    if(this.fileMissionType == '任务完成') {
      this.getDoneMissionDetail();
    }else {
