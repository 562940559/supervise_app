...
export default {
  ...
  methods: {
    ...
    getTodo() {
      ...
      this.$api.zprwFileMission.getFileMission(data)
      .then(res=>{
        ...
        this.info.current++;
        this.todo = this.todo.concat(res.data);
        this.loading = false;
      })
    },
    getDone() {
      ...
      var info = aes.encrypt(JSON.stringify(this.info)),
      data = `"requestData": "${info}"`;
      this.$api.zprwFileMission.getDoneFileMission(data)
      .then(res=>{
        if (res.data[0] == undefined) {
          this.loading = false;
          this.finished = true;
        ...
        this.info.current++;
        this.done = this.done.concat(res.data);
        this.loading = false;
      })
    },
    onclick(name,title) {
