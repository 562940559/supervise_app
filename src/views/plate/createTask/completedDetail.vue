<template>
  <div class="completedDetailWrap">
    <div class="plateWrap">
      <span>督办事项</span>
      <span @click="showPop2(detailInfo.supervisoryItems)">{{detailInfo.supervisoryItems}}</span>
    </div>
    <div class="groupWrap">
      <div class="plateWrap">
        <span>开始时间</span>
    ...
    <div class="groupWrap">
      <div class="plateWrap">
        <span>任务来源</span>
        <span @click="showPop2(detailInfo.source)">{{detailInfo.source}}</span>
      </div>
      <div @click="routerPush('fileBasis')" class="plateWrap">
        <span>查看文件依据</span>
        <span><van-icon name="arrow" /></span>
      </div>
      <div @click="routerPush('seeCycle')" class="plateWrap">
        <span>查看汇报周期</span>
        <span><van-icon name="arrow" /></span>
      </div>
      <div class="plateWrap">
    ...
    <div class="groupWrap">
      <div class="plateWrap">
        <span>牵头负责人</span>
        <span>{{detailInfo.takeleadStaff}}</span>
      </div>
      <div class="plateWrap">
        <span>配合处室负责人</span>
        <span>{{detailInfo.responsibilityStaff}}</span>
      </div>
      <div class="plateWrap">
        <span>分管领导</span>
        <span>{{detailInfo.leading}}</span>
      </div>
      <div class="plateWrap">
        <span>督办任务审批人</span>
        <span>{{detailInfo.approver}}</span>
      </div>
    </div>
    <div class="groupWrap">
      ...
      <div @click="routerPush('examineDetail')" class="plateWrap">
        <span>查看审核详情</span>
        <span><van-icon name="arrow" /></span>
      </div>
...
export default {
  ...
  methods: {
    showPop(where) {
      ...
      this.showText = where == 'content' ? this.detailInfo.supervisoryContent : this.detailInfo.remark
      this.show = true;
    },
    showPop2(item) {
      this.showTitle = '';
