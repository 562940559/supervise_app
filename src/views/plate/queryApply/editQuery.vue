<template>
  <div class="addQueryWrap">
    ...
    <!-- 提交按钮 -->
    <van-button type="primary" size="large" @click="submitForm()">修改</van-button>
    <!-- pop弹出层 -->
...
export default {
  ...
  activated() {
    if (refreshFlag) {
      ...
      this.$api.queryApply.getEditDetail(this.applyId).then(res => {
        this.personInfo.applyDept = res.data.applyDept;
        this.personInfo.applicant = res.data.applicant;
        this.personInfo.phone = res.data.phone;
        this.para.dataRequirements = res.data.dataRequirements;
        this.para.queryProject = res.data.queryProject;
      });
    } else {
      this.para.dataRequirements = this.$route.query.haveSelected;
    }
  },
  methods: {
    routerBack() {
    ...
    submitForm() {
      ...
      this.$api.queryApply.editApply(this.applyId,this.para).then(res => {
        if (res.code == "200") {
          Toast("申请修改成功！");
          setTimeout(() => {
            this.$router.back("-1");
