    ...
    <van-tabs @click="onclick" v-model="active">
      <van-tab title="待我汇报">
        <div class="containerWrap">
          ...
          >
            <div class="titleWrap">
              <p>督办事项：{{item.superviseHandleMatter}}</p>
              <span 
                @click="goOther('/coordinateReport',item.id,'report')" 
                v-if="item.performOperations == '汇报'">
                <b class="report">汇报</b>
              </span>
              <span
                @click="goOther('/coordinateReport',item.id,'modify')"
                v-else
              ><b class="modify">修改</b></span>
            </div>
            <div @click="goOther('/coordinateReportDetail',item.id,'detail')" class="content">
              <p><span>汇报截止时间：</span><b>{{item.appointedTaskReportTime}}</b></p>
...
export default {
  ...
  methods: {
    ...
    reportBack(id,index) {
      this.$api.coordinationMission.reportBack(id)
      .then(res=>{
        if(res.code == '200') {
    ...
    getInReport() {
      ...
      this.$api.coordinationMission.getInReport(data)
      .then(res=>{
    ...
    getReported() {
      ...
      var info = aes.encrypt(JSON.stringify(this.info)),
      data = `"requestData": "${info}"`;
      this.$api.coordinationMission.getReported(data)
      .then(res=>{
