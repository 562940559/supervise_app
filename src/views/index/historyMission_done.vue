<template>
  <div class="historyMission-wrap">
    <div class="top-wrap">
      ...
      <p>
        <router-link style="color: #ABABAB" to="/historyMission" replace>
          <span>进行中</span>
        </router-link>
        <router-link to="/historyMission_done" replace>
          <span>已完成</span>
        </router-link>
      </p>
    </div>
...
export default {
  ...
  methods: {
    ...
    getHistoryMissionList() {
      ...
      .then(res=>{
        if (JSON.stringify(res.completeData[0]) == undefined) {
          this.loading = false;
          this.finished = true;
          return false;
        }
        this.historyMissionList = this.historyMissionList.concat(res.completeData);
